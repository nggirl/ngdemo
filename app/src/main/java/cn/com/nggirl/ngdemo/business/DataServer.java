...
import cn.com.nggirl.ngdemo.keyboard.MultipleItem;
...
public class DataServer {
    ...
    public static List<MultiStatus> getSampleMultiData(int lenth) {
        List<MultiStatus> list = new ArrayList<>();
        MultiStatus statusHeader = new MultiStatus(MultiStatus.ITEM_HEADER);
        statusHeader.setUserName("Chad" + 0);
        statusHeader.setCreatedAt("04/05/" + 0);
        statusHeader.setRetweet(0 % 2 == 0);
        statusHeader.setUserAvatar("https://avatars1.githubusercontent.com/u/7698209?v=3&s=460");
        statusHeader.setText("BaseRecyclerViewAdpaterHelper https://www.recyclerview.org");
        list.add(statusHeader);
        for (int i = 0; i < lenth; i++) {
            MultiStatus status = new MultiStatus(MultiStatus.ITEM_ITEM);
            status.setUserName("Chad" + i);
            status.setCreatedAt("04/05/" + i);
            status.setRetweet(i % 2 == 0);
            status.setUserAvatar("https://avatars1.githubusercontent.com/u/7698209?v=3&s=460");
